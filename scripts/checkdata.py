...
def err(line_no, msg):
    logging.info("line {0}: {1}".format(line_no, msg))
...
def main():
    if len(argv) != 2:
        logging.info("Usage: {0} dataset".format(argv[0]))
        exit(1)

    dataset = argv[1]

    if not os.path.exists(dataset):
        logging.info("dataset {0} not found".format(dataset))
        exit(1)
    ...
    if error_line_count > 0:
        logging.warning("Found {0} lines with error.".format(error_line_count))
        return 1
    else:
