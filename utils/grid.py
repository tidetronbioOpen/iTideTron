...
class GridOption:
    def __init__(self, dataset_pathname, options):
        ...
        if sys.platform != 'win32':
            ...
            self.gnuplot_pathname = None #'/usr/bin/gnuplot'
        else:
            # example for windows
            self.svmtrain_pathname = os.path.join(dirname, r'..\svm-train.exe')
    ...
    def run(self):
        while True:
            ...
            if cexp is WorkerStopToken:
                ...
                # logging.info('worker {0} stop.'.format(self.name))
                break
            try:
                c, g = None, None
                if cexp != None:
...
class TelnetWorker(Worker):
    ...
    def run(self):
        ...
        #
        logging.info('login ok', self.host)
