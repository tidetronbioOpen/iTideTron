...
import numpy as np
from nnet.charbilstm import CharBiLSTM
from nnet.charbigru import CharBiGRU
from nnet.charcnn import CharCNN


class WordRep(nn.Module):
    """
    WordRep类，主要是作为字符处理类的综合，
    引用了CharBiLSTM类，CharCNN类以及CharBiGRU类实例及函数等,
    可以建立随机的嵌入模型，
    也可以根据输入的预训练嵌入模型对序列进行处理，
    生成词嵌入向量。
    """
