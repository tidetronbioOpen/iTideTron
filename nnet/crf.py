...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        for idx in range(len(back_points)-2, -1, -1):
            ...
            # print(decode_idx[98].shape)
            # print(torch.squeeze(pointer.data).shape)
            # decode_idx[idx] = pointer.data
            decode_idx[idx] = torch.squeeze(pointer.data)
            #也可以用 decode_idx[idx] = pointer.detach().view(batch_size)
        path_score = None
    ...
    def _viterbi_decode_nbest(self, feats, mask, nbest):
        ...
        for idx, cur_values in seq_iter:
            ...
            ## set padded label as 0, which will be filtered in post processing
            ## mask[idx] ? mask[idx-1]
            cur_bp.masked_fill_(mask[idx].view(batch_size, 1, 1).expand(batch_size, tag_size, nbest).bool(), 0)
            # print cur_bp[0]
            back_points.append(cur_bp)
        ### add score to final STOP_TAG
